...
async def name_clarify(message: types.Message, state: FSMContext):
    if message.text == "Да":
        await bot.send_message(message.chat.id, get_name_set_message(), reply_markup=types.ReplyKeyboardRemove())
        await FSMGameInit.get_players_num.set()
        await bot.send_message(message.chat.id, get_players_num_message(), reply_markup=get_players_num_keyboard())
    elif message.text == "Нет":
        await bot.send_message(message.chat.id, get_name_not_set_message(), reply_markup=types.ReplyKeyboardRemove())
        await FSMGameInit.get_name.set()
    else:
...
async def get_players_num(callback: types.callback_query, state: FSMContext):
    players_num = callback.data.split("_")[-1]
...
def register_get_name_handlers(dp: Dispatcher):
